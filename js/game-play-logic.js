...
function dealCard(hand, location) {
  ...
  var cardImage = $('<img>')
    .attr('class', 'card')
    .attr('src', hand[index].src)
    .hide();
  cardImage.attr('id', currentTurn + '-card-' + index);
  cardImage.attr('data-rank', hand[index].label);
  cardImage.attr('data-suit-glyph', hand[index].suitGlyph);
  ...
  if (dealerHand.length === 2 && currentTurn === 'dealer') {
    cardImage.addClass('card-back');
    cardImage.attr('data-rank', '');
    cardImage.attr('data-suit-glyph', '');
...
function evaluateGameStatus() {
  ...
  if (currentTurn != 'dealer') {
    if (playerHasAce === true) {
      if (currentTurn === 'player') {
        // Dry out by having params in here
...
function isPlayerDone() {
  if (splitGame === false && playerHandTotal >= 21) {
    ...
    if (currentTurn === 'player') {
      ...
      } else if (playerHandTotal > 21) changeHand(playerStatus);
    } else if (currentTurn === 'playerSplit') {
